...
while IFS= read -r -d '' cfg_file; do
	...
	if grep -q "aspect_ratio_index" "$cfg_file" && ! grep -qE "aspect_ratio_index = \"(22|24)\"" "$cfg_file"; then
		...
		/mnt/SDCARD/System/usr/miyoo/scripts/patch_ra_cfg.sh "$configPatchFile" "$cfg_file"
...
/mnt/SDCARD/System/usr/miyoo/scripts/mainui_state_update.sh "OVERLAYS" "$script_name"
