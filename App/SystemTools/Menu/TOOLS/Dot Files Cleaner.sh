...
/mnt/SDCARD/System/usr/miyoo/scripts/infoscreen.sh -m "Cleaning dot files... Please wait."
...
button=$(/mnt/SDCARD/System/usr/miyoo/scripts/infoscreen.sh -m "Dot clean done. Press X to run a full clean on the SD card in background (very long). Press B to Quit" -fs 28 -k "X B MENU")
if [ "$button" = "X" ]; then
    (
        /mnt/SDCARD/System/usr/miyoo/scripts/infoscreen.sh -m "Cleaning dot files in background."
