...
database_file="/mnt/SDCARD/App/SystemTools/Menu/Menu_cache6.db"
CurrentTheme=$(/usr/miyoo/bin/jsonval Theme)
CrossMix_Style=$(/mnt/SDCARD/System/bin/jq -r '.["CROSSMIX STYLE"]' "/mnt/SDCARD/System/etc/surwish.json")
mkdir -p /mnt/SDCARD/System/starts/
mkdir -p /mnt/SDCARD/System/etc
read -r device_type </userdata/current_device.txt

if [ ! -f "/mnt/SDCARD/System/etc/surwish.json" ]; then
  touch "/mnt/SDCARD/System/etc/surwish.json"
fi
sync
...
/mnt/SDCARD/System/usr/miyoo/scripts/infoscreen.sh -m "$LaunchMessage"

read -r device_type </userdata/current_device.txt

cp /mnt/SDCARD/App/SystemTools/GoTo_SystemTools_List_$device_type.json /tmp/state.json
sync
...
if [ -d "/mnt/SDCARD/App/SystemTools/Menu/Imgs_$CrossMix_Style" ]; then
  img_path="/mnt/SDCARD/App/SystemTools/Menu/Imgs_$CrossMix_Style"
else
  img_path="/mnt/SDCARD/App/SystemTools/Menu/Imgs"
fi

# =========================================== Populate icons, backgrounds and themes sets ============================================

# We remove dot files from Mac OS
find /mnt/SDCARD/App/SystemTools -name '._*' -exec rm '{}' \;

BG_list_directory="/mnt/SDCARD/App/SystemTools/Menu/ADVANCED SETTINGS##BACKGROUNDS (value)/"
...
ICON_list_directory="/mnt/SDCARD/App/SystemTools/Menu/ADVANCED SETTINGS##ICONS (value)/"
...
THEME_list_directory="/mnt/SDCARD/App/SystemTools/Menu/ADVANCED SETTINGS##THEMES (value)/"
...
search_path="/mnt/SDCARD/App/SystemTools/Menu"

for folder in "$search_path"/*/; do
  if ! [ -f "$folder/.no_$device_type" ]; then
    ...
    subfolder_query="INSERT INTO Menu_roms (disp, path, imgpath, type, ppath, pinyin, cpinyin, opinyin) VALUES (\"$folder_escaped\", \"${folder%/}\", \"${img_path}/${folder_name}.png\", 1, \".\", \"\", \"\", \"\")"
...
find "$search_path" -mindepth 1 -maxdepth 2 -type f -name "*.sh" | while read -r file; do
  ...
  if ! [ -f "$subfolder/.${filename_without_ext}.no_$device_type" ]; then
...
sqlite3 "$database_file" "SELECT disp, path FROM Menu_roms WHERE type = 1 AND disp LIKE '% (state)' ;" |
  while IFS='|' read -r disp path; do
    ...
    CurState=$(jq -r --arg disp "$disp_withoutstate" '.[$disp]' "/mnt/SDCARD/System/etc/surwish.json")
...
sqlite3 "$database_file" "SELECT disp, path FROM Menu_roms WHERE type = 1 AND disp LIKE '% (value)' ;" |
  while IFS='|' read -r disp path; do
    ...
    if [ "$disp_withoutvalue" = "THEMES" ]; then
      ...
      CurState=$(jq -r --arg disp "$disp_withoutvalue" '.[$disp] // "Default"' "/mnt/SDCARD/System/etc/surwish.json")
    fi
