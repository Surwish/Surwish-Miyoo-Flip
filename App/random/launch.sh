...
choose_random_db() {
  find /mnt/SDCARD/Roms -mindepth 1 -maxdepth 2 -type f -name '*_cache6.db' ! -path '*/_*/*' ! -path '*/MUSIC/*' ! -path '*/VIDEOS/*' | shuf -n 1
}
...
main() {
  if [ -f "./random_manual.png" ]; then
    /mnt/SDCARD/System/usr/miyoo/scripts/infoscreen.sh -i "./random_manual.png" -k "A B Y X L R SELECT START MENU"
  ...
  while true; do
    ...
    if [ -f "$path" ]; then
      ...
      if ! grep -q "$path" "$presented_games_file"; then
        ...
        button=$(/mnt/SDCARD/System/usr/miyoo/scripts/infoscreen.sh -i "$imgtodisplay" -c "122,122,122" -m "${disp}" -k "A B X MENU")

        if [ "$button" = "A" ]; then
          echo "Okay, launching the game: $disp"

          launcher=$(jq -r '.launch' "/mnt/SDCARD/Emu/$table_prefix/config.json")
          launchpath="/mnt/SDCARD/Emu/$table_prefix/$launcher"
          ...
          /mnt/SDCARD/System/usr/miyoo/scripts/infoscreen.sh -c "122,122,122" -m "Exiting"
          exit 0
        fi
