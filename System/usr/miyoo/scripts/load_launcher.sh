...
export LD_LIBRARY_PATH="/usr/miyoo/lib:${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
...
echo "load_launcher.sh : $Launcher_name dowork 0x"  
"$EMU_DIR/$Launcher_command" "$@" >> /tmp/test.txt


echo "$EMU_DIR/$Launcher_command $@" >> /tmp/test.txt
