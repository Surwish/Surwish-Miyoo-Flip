...
determine_image_path() {
    ...
    image_name="$1"
    base_path="/mnt/SDCARD/System/resources/surwish"
...
# echo "--------------------------------------------"
# echo /mnt/SDCARD/System/bin/sdl2imgshow \
    # -i "$image" \
    # -f "$font_file" \
    # -s "$font_size" \
    # -c "$color" \
...
# /mnt/SDCARD/System/bin/sdl2imgshow -i "$image" -f /mnt/SDCARD/System/resources/DejaVuSans.ttf -s 22 -c 220,220,220 -t Current IP address: /mnt/SDCARD/System/resources/surwish/bg-exit_miyooflip.png -f /mnt/SDCARD/System/resources/DejaVuSans.ttf -s 22 -c 220,220,220 -t "Current IP address:"
...
if [ -n "$wait_keys" ]; then
    handle_timer &
    handle_timer_pid=$!
    button=$(/mnt/SDCARD/System/usr/miyoo/scripts/getkey.sh "$wait_keys" 2>/dev/null)
    if [ -z "$button" ]; then
