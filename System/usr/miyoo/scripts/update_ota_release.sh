...
get_release_info() {
    ...
    Local_CrossMixVersion="$(cat /mnt/SDCARD/System/usr/miyoo/crossmix-version.txt)"
...
download_update() {
    Mychoice=$(echo -e "No\nYes" | /mnt/SDCARD/System/usr/miyoo/scripts/shellect.sh -t "Download v$Release_Version (${Release_size_MB}MB) ?" -b "Press A to validate your choice.")
...
apply_update() {
    Mychoice=$(echo -e "No\nYes" | /mnt/SDCARD/System/usr/miyoo/scripts/shellect.sh -t "Apply update $Release_Version ?" -b "Press A to validate your choice.")
    clear
    if [ "$Mychoice" = "Yes" ]; then
        ...
        DownloadPath="/mnt/SDCARD/System/usr/miyoo/scripts/crossmix_update.sh"
